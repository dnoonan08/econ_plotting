...
def pll_scan_plots(fname,
    ...
    if 'json' in fname:
        ...
        lockState = None
        autolock_capIndex = None
        for t in data['tests']:
            if 'test_pll2D_scan' in t['nodeid']:
                freq = np.array(t['metadata']['frequencies_used'])
                lockState = np.array(t['metadata']['locks'])
                autolock_vco = np.array(t['metadata']['autolock_capbank'])
                autolock_lockstate = np.array(t['metadata']['autolock_locked'])

                autolock_capIndex = allowed_cap_bank_vals.searchsorted(autolock_vco)[autolock_lockstate==1]
                autolock_freq     = freq[autolock_lockstate==1]
        ...
        if not autolock_capIndex is None:
